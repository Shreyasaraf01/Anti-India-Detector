...
def predict_fake(text: str) -> dict:
    """
    Predict whether given text is fake, real, or propaganda.
    Returns dict with human-readable label + probability.
    """
    ...
    try:
        ...
        # Map numeric label to human-readable label
        label_map = {0: "real", 1: "fake", 2: "propaganda"}
        label_name = label_map.get(pred, str(pred))

        return {"label": label_name, "probability": float(prob)}
    except Exception as e:
        return {"error": str(e)}
